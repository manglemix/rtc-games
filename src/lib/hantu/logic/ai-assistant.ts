...
`;
		...
		const otherPlayers = Array.from(
			this.gameState.players
				.values()
				.filter((player) => player.name !== this.gameState.thisPlayer.name)
		);
		if (this.gameState.thisPlayer.possessed) {
			if (this.gameState.initialPossessedCount > 1) {
				msg +=
					'The other possessed players are: ' +
					otherPlayers
						.filter((player) => player.possessed)
			...
			msg +=
				'The other players are: ' + otherPlayers.map((player) => player.name).join(', ') + '.\n';
		}

		const proposals = Array.from(this.gameState.proposals)
			.map((name) => {
	...
	constructor(
		...
		netClient.addOnMessage('ollama', (from, msg) => {
			if (!this.ollamaInitialized) {
				...
				this.netClient.sendTo(
					from,
					JSON.stringify({ part: "I'm sorry, the host does not seem to have ollama set up." }),
					'ollama'
				);
