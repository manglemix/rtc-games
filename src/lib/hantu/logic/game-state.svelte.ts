...
import { generateContextless, listModels, pullModel } from '$lib/ollama';
...
If 5 nights have passed, the non-possessed automatically lose. If the crypt is fixed, everyone votes on who they think is possessed. The top ${this.initialPossessedCount} most-voted
players are killed where ${this.initialPossessedCount} is the number of possessed at the start of the game. If all possessed players are dead, the non-possessed win, and vice-versa.
Dead players can only spectate.

The current player you are helping is named "${this.thisPlayer.name}", is ${this.thisPlayer.possessed ? 'possessed' : 'not possessed'}, and is ${this.thisPlayer.alive ? 'alive' : 'not alive'}.
`;

		const otherPlayers = Array.from(this.players.values().filter((player) => player.name !== this.thisPlayer.name));
		if (this.thisPlayer.possessed) {
			if (this.initialPossessedCount > 1) {
		...
		const proposals = Array.from(this.proposals).join(', ');
		switch (this.state) {
			case State.KeyProposition:
				msg += `It is currently the key proposition phase. ${this.proposer.name} is the proposer. ${this.proposals.size > 0 ? `The current proposed key holders are: ${proposals}` : 'They have not proposed anyone yet.'}.\n`;
				break;
			case State.KeyVote:
...
export class HostGameState extends GameState {
		...
		listModels().then(
			async (models) => {
				if (!models.includes('llama3.1:8b')) {
					...
					const status = await pullModel('llama3.1:8b');
					if (status !== 'success') {
	...
	public askAI(prompt: string): void {
		generateContextless(
			...
			this.onAIResponsePart("Model is not initialized");
