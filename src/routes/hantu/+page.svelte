...
{#if onMainMenu}
    ...
    <section id="main-menu" class="flex flex-col items-center">
        ...
        <input type="text" id="name"  disabled={joining || netClient !== null} bind:value={name} maxlength="12" />

        <label class="ml-8 mt-8 inline-flex cursor-pointer items-center">
            <input
                type="checkbox"
                class="peer sr-only"
                onclick={() => isHost = !isHost}
            />
            <div
        ...
        {:else}
            ...
            <input disabled={joining || netClient !== null} type="text" id="roomCode" maxlength="6" bind:value={roomCode} />
            <button disabled={joining} onclick={async () => {
                ...
                joining = true;
                netClient = await defaultConnectToRoom(
                    "hantu",
                    roomCode,
                    name,
                    DATA_CHANNELS,
