...
export const POST: RequestHandler = async ({ params, request }) => {
    ...
    await rtc_kv.set(`advertisements:${game}:${roomCode}`, JSON.stringify(advertisement), { EX: 1800 });
...
export const GET: RequestHandler = async ({ params, request }) => {
    ...
    await rtc_kv.connect();
    const advertisementStr: string = await rtc_kv.get(`advertisements:${game}:${roomCode}`) ?? '{ "peerNames": [], "hostName": "" }';
    // const advertisement: { peerNames: string[], hostName: string } = await JSON.parse(advertisementStr);

	return new Response(advertisementStr, { status: 200 });
};
